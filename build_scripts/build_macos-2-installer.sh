...
LAST_EXIT_CODE=$?
ls -l dist/mac*/dogechia.app/Contents/Resources/app.asar
...
DMG_NAME="dogechia-${DOGECHIA_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/dogechia-"${DOGECHIA_INSTALLER_VERSION}"-arm64.dmg
  DMG_NAME=dogechia-${DOGECHIA_INSTALLER_VERSION}-arm64.dmg
fi
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id net.dogechia.blockchain \
...
# xcrun altool --notarize-app -f Dogechia-0.1.X.dmg --primary-bundle-id net.dogechia.blockchain -u username -p password
...
# xcrun stapler staple Dogechia-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Dogechia-0.1.X.dmg
