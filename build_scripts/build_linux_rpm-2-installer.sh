...
# Builds CLI only rpm
CLI_RPM_BASE="doge-chia-cli-$DOGECHIA_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/dogechia"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/dogechia/"
ln -s ../../opt/dogechia/dogechia "dist/$CLI_RPM_BASE/usr/bin/dogechia"
...
fpm -s dir -t rpm \
  ...
  --name doge-chia-cli \
  --license Apache-2.0 \
  --version "$DOGECHIA_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Dogechia is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  ...
  --config.rpm.packageName="doge-chia"
...
GUI_RPM_NAME="doge-chia-${DOGECHIA_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${DOGECHIA_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
