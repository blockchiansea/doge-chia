...
git submodule update --init dogechia-blockchain-gui

Set-Location -Path ".\dogechia-blockchain-gui" -PassThru
...
# Other than `dogechia-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @dogechia-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@dogechia-network` because it causes an error on later `electron-packager` command
Remove-Item "@dogechia-network" -Recurse -Force
