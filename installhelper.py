...
# Gets the version from the result of "dogechia version"
...
def get_dogechia_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["dogechia", "version"], capture_output=True)
    if output.returncode == 0:
        version = str(output.stdout.strip(), "utf-8").splitlines()[-1]
    return make_semver(version.replace("H", ""))
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_dogechia_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/dogechia-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/dogechia-blockchain-gui/packages/gui/package.json")
