...
if [ "$(uname -m)" = "armv7l" ]; then
  ...
  echo "https://github.com/blockchiansea/doge-chia/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
# Get submodules
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with dogechia
  ...
  # yum install python3 brings Python3.6 which is not supported by dogechia
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "12" ]; then
  echo "Dogechia requires Python version >= 3.7 and  < 3.12.0" >&2
...
echo "https://discord.gg/dogechia"
echo ""
echo "Try the Quick Start Guide to running doge-chia:"
echo "https://github.com/blockchiansea/doge-chia/wiki/Quick-Start-Guide"
echo ""
