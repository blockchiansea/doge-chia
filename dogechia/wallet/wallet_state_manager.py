...
from dogechia.consensus.coinbase import farmer_parent_id, pool_parent_id
...
from dogechia.data_layer.dl_wallet_store import DataLayerStore
from dogechia.pools.pool_puzzles import (
    SINGLETON_LAUNCHER_HASH,
    get_most_recent_singleton_coin_from_coin_spend,
    solution_to_pool_state,
)
from dogechia.pools.pool_wallet import PoolWallet
from dogechia.protocols.wallet_protocol import CoinState, NewPeakWallet
...
from dogechia.util.db_wrapper import DBWrapper2
...
from dogechia.util.misc import UInt32Range, UInt64Range, VersionedBlob
from dogechia.util.path import path_from_root
from dogechia.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from dogechia.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from dogechia.wallet.cat_wallet.cat_wallet import CATWallet
from dogechia.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from dogechia.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from dogechia.wallet.key_val_store import KeyValStore
from dogechia.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
...
from dogechia.wallet.notification_manager import NotificationManager
from dogechia.wallet.outer_puzzles import AssetType
from dogechia.wallet.payment import Payment
from dogechia.wallet.puzzle_drivers import PuzzleInfo
from dogechia.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
from dogechia.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from dogechia.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from dogechia.wallet.singleton import create_singleton_puzzle
from dogechia.wallet.trade_manager import TradeManager
...
from dogechia.wallet.util.address_type import AddressType
...
from dogechia.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from dogechia.wallet.util.query_filter import HashFilter
from dogechia.wallet.util.transaction_type import CLAWBACK_TRANSACTION_TYPES, TransactionType
from dogechia.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from dogechia.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from dogechia.wallet.vc_wallet.vc_drivers import VerifiedCredential
from dogechia.wallet.vc_wallet.vc_store import VCStore
from dogechia.wallet.vc_wallet.vc_wallet import VCWallet
from dogechia.wallet.wallet import Wallet
from dogechia.wallet.wallet_blockchain import WalletBlockchain
from dogechia.wallet.wallet_coin_record import WalletCoinRecord
from dogechia.wallet.wallet_coin_store import WalletCoinStore
from dogechia.wallet.wallet_info import WalletInfo
from dogechia.wallet.wallet_interested_store import WalletInterestedStore
from dogechia.wallet.wallet_nft_store import WalletNftStore
from dogechia.wallet.wallet_pool_store import WalletPoolStore
from dogechia.wallet.wallet_protocol import WalletProtocol
from dogechia.wallet.wallet_puzzle_store import WalletPuzzleStore
from dogechia.wallet.wallet_retry_store import WalletRetryStore
from dogechia.wallet.wallet_transaction_store import WalletTransactionStore
from dogechia.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSDogechiaConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(self, clawback_coins: Dict[Coin, ClawbackMetadata], fee: uint64) -> List[bytes32]:
        ...
        if fee > 0:
            dogechia_tx = await self.main_wallet.create_tandem_xdg_tx(
                fee, Announcement(coin_spends[0].coin.name(), message)
            )
            assert dogechia_tx.spend_bundle is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xdg_spam_amount = self.config.get("xdg_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xdg_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xdg_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xdg_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSDogechiaConnection) -> Optional[bytes32]:
        # Get minter DID
