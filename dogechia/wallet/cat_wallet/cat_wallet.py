...
from dogechia.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from dogechia.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            dogechia_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        dogechia_tx = dataclasses.replace(dogechia_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(dogechia_tx)
    ...
    async def create_tandem_xdg_tx(
        ...
        if fee > amount_to_claim:
            dogechia_coins = await self.standard_wallet.select_coins(
                fee,
                min_coin_amount=min_coin_amount,
                max_coin_amount=max_coin_amount,
                excluded_coin_amounts=exclude_coin_amounts,
            )
            origin_id = list(dogechia_coins)[0].name()
            ...
            message = None
            for spend in dogechia_tx.spend_bundle.coin_spends:
            ...
            selected_amount = sum([c.amount for c in dogechia_coins])
            dogechia_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return dogechia_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XDG as part of this
        regular_dogechia_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_dogechia_to_claim = payment_amount

        need_dogechia_transaction = (fee > 0 or regular_dogechia_to_claim > 0) and (fee - regular_dogechia_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_dogechia_transaction:
                    if fee > regular_dogechia_to_claim:
                        dogechia_tx, _ = await self.create_tandem_xdg_tx(
                            fee,
                            uint64(regular_dogechia_to_claim),
                    ...
                    elif regular_dogechia_to_claim > fee:
                        dogechia_tx, _ = await self.create_tandem_xdg_tx(
                            fee,
                            uint64(regular_dogechia_to_claim),
                            min_coin_amount=min_coin_amount,
        ...
        dogechia_spend_bundle = SpendBundle([], G2Element())
        if dogechia_tx is not None and dogechia_tx.spend_bundle is not None:
            dogechia_spend_bundle = dogechia_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    dogechia_spend_bundle,
                ]
            ),
            dogechia_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        if dogechia_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=dogechia_tx.confirmed_at_height,
                    created_at_time=dogechia_tx.created_at_time,
                    to_puzzle_hash=dogechia_tx.to_puzzle_hash,
                    amount=dogechia_tx.amount,
                    fee_amount=dogechia_tx.fee_amount,
                    confirmed=dogechia_tx.confirmed,
                    sent=dogechia_tx.sent,
                    spend_bundle=None,
                    additions=dogechia_tx.additions,
                    removals=dogechia_tx.removals,
                    wallet_id=dogechia_tx.wallet_id,
                    sent_to=dogechia_tx.sent_to,
                    trade_id=dogechia_tx.trade_id,
                    type=dogechia_tx.type,
                    name=dogechia_tx.name,
                    memos=[],
                )
            )
