...
from dogechia.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from dogechia.protocols import wallet_protocol
...
from dogechia.util.errors import Err, ValidationError
from dogechia.util.ints import uint32, uint128
...
from dogechia.util.path import mkdir, path_from_root
from dogechia.wallet.block_record import HeaderBlockRecord
from dogechia.wallet.derivation_record import DerivationRecord
from dogechia.wallet.settings.settings_objects import BackupInitialized
from dogechia.wallet.transaction_record import TransactionRecord
from dogechia.wallet.util.backup_utils import open_backup_file
from dogechia.wallet.util.wallet_types import WalletType
from dogechia.wallet.wallet_action import WalletAction
from dogechia.wallet.wallet_blockchain import ReceiveBlockResult
from dogechia.wallet.wallet_state_manager import WalletStateManager
from dogechia.util.profiler import profile_task
...
class WalletNode:
    ...
    server: Optional[DogeChiaServer]
    log: logging.Logger
    ...
    def set_server(self, server: DogeChiaServer):
        self.server = server
    ...
    async def on_connect(self, peer: WSDogeChiaConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSDogeChiaConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSDogeChiaConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSDogeChiaConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSDogeChiaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSDogeChiaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
