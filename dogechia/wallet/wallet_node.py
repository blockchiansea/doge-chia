...
from dogechia.consensus.blockchain import AddBlockResult
from dogechia.consensus.constants import ConsensusConstants
from dogechia.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from dogechia.full_node.full_node_api import FullNodeAPI
from dogechia.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from dogechia.rpc.rpc_server import StateChangedProtocol, default_get_connections
from dogechia.server.node_discovery import WalletPeers
...
from dogechia.types.weight_proof import WeightProof
from dogechia.util.chunks import chunks
...
from dogechia.util.db_wrapper import manage_connection
from dogechia.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from dogechia.wallet.puzzles.clawback.metadata import AutoClaimSettings
from dogechia.wallet.transaction_record import TransactionRecord
from dogechia.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from dogechia.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from dogechia.wallet.wallet_state_manager import WalletStateManager
from dogechia.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[DogechiaServer] = None
    ...
    def get_cache_for_peer(self, peer: WSDogechiaConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'dogechia keys' program.")
            return None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.dogechi.org"]),
    ...
    def on_disconnect(self, peer: WSDogechiaConnection) -> None:
    ...
    async def on_connect(self, peer: WSDogechiaConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    async def long_sync(
        ...
        full_node: WSDogechiaConnection,
        fork_height: int,
        *,
        rollback: bool,
    ) -> None:
    ...
    async def is_peer_synced(self, peer: WSDogechiaConnection, height: uint32) -> Optional[uint64]:
        # Get last timestamp
    ...
    def is_trusted(self, peer: WSDogechiaConnection) -> bool:
    ...
    async def state_update_received(self, request: CoinStateUpdate, peer: WSDogechiaConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSDogechiaConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSDogechiaConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSDogechiaConnection]:
        """
        ...
        synced_and_trusted: List[WSDogechiaConnection] = []
        synced: List[WSDogechiaConnection] = []
        trusted: List[WSDogechiaConnection] = []
        neither: List[WSDogechiaConnection] = []
        all_nodes: List[WSDogechiaConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSDogechiaConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSDogechiaConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSDogechiaConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSDogechiaConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSDogechiaConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSDogechiaConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSDogechiaConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSDogechiaConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSDogechiaConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSDogechiaConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
