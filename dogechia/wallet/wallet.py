...
from dogechia.wallet.secret_key_store import SecretKeyStore
...
CHIP_0002_SIGN_MESSAGE_PREFIX = "Dogechia Signed Message"
...
class Wallet:
    ...
    async def create_tandem_xdg_tx(
        ...
        dogechia_coins = await self.select_coins(fee)
        if reuse_puzhash is None:
            reuse_puzhash_config = self.wallet_state_manager.config.get("reuse_public_key_for_change", None)
            if reuse_puzhash_config is None:
                reuse_puzhash = False
        ...
        dogechia_tx = await self.generate_signed_transaction(
            uint64(0),
            (await self.get_puzzle_hash(not reuse_puzhash)),
            fee=fee,
            coins=dogechia_coins,
    ...
    # WSDogechiaConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSDogechiaConnection
