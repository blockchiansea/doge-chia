...
from dogechia.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import solution_for_conditions
...
from dogechia.wallet.vc_wallet.vc_store import VCRecord, VCStore
...
if TYPE_CHECKING:
    from dogechia.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def generate_signed_transaction(
        ...
        if fee > 0:
            ...
            dogechia_tx = await self.wallet_state_manager.main_wallet.create_tandem_xdg_tx(
        ...
        if dogechia_tx is not None and dogechia_tx.spend_bundle is not None:
            spend_bundles.append(dogechia_tx.spend_bundle)
            tx_list.append(dataclasses.replace(dogechia_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        self, parent_id: bytes32, peer: WSDogechiaConnection, fee: uint64 = uint64(0), reuse_puzhash: Optional[bool] = None
    ) -> List[TransactionRecord]:
        ...
        if fee > 0:
            dogechia_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_xdg_tx(
                fee, vc_announcement, reuse_puzhash
            )
            assert tx.spend_bundle is not None
            assert dogechia_tx.spend_bundle is not None
            tx = dataclasses.replace(tx, spend_bundle=SpendBundle.aggregate([dogechia_tx.spend_bundle, tx.spend_bundle]))
            dogechia_tx = dataclasses.replace(dogechia_tx, spend_bundle=None)
            return [tx, dogechia_tx]
        else:
...
if TYPE_CHECKING:
    from dogechia.wallet.wallet_protocol import WalletProtocol  # pragma: no cover
