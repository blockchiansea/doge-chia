...
import dogechia.wallet.singleton
from dogechia.protocols.wallet_protocol import CoinState
...
from dogechia.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from dogechia.util.hash import std_hash
from dogechia.util.ints import uint16, uint32, uint64, uint128
from dogechia.wallet.derivation_record import DerivationRecord
from dogechia.wallet.did_wallet import did_wallet_puzzles
from dogechia.wallet.did_wallet.did_info import DIDInfo
from dogechia.wallet.lineage_proof import LineageProof
from dogechia.wallet.nft_wallet import nft_puzzles
from dogechia.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from dogechia.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from dogechia.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from dogechia.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from dogechia.wallet.payment import Payment
from dogechia.wallet.puzzle_drivers import PuzzleInfo, Solver
from dogechia.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from dogechia.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from dogechia.wallet.uncurried_puzzle import uncurry_puzzle
from dogechia.wallet.util.compute_memos import compute_memos
from dogechia.wallet.util.transaction_type import TransactionType
from dogechia.wallet.util.wallet_sync_utils import fetch_coin_spend
from dogechia.wallet.util.wallet_types import WalletType
from dogechia.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
from dogechia.wallet.wallet_coin_record import WalletCoinRecord
from dogechia.wallet.wallet_info import WalletInfo
from dogechia.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    if TYPE_CHECKING:
        from dogechia.wallet.wallet_protocol import WalletProtocol
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSDogechiaConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSDogechiaConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, dogechia_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if dogechia_tx is not None and dogechia_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, dogechia_tx.spend_bundle])
            dogechia_tx = dataclasses.replace(dogechia_tx, spend_bundle=None)
        ...
        if dogechia_tx is not None:
            tx_list.append(dogechia_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            dogechia_tx = await self.standard_wallet.create_tandem_xdg_tx(
        ...
        return nft_spend_bundle, dogechia_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XDG'} to offer")
    ...
    async def mint_from_did(
        ...
        xdg_coins: Optional[Set[Coin]] = None,
        xdg_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xdg spend that funds the transactions and pays fees.
        ...
        :param xdg_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xdg_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xdg spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xdg coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xdg_coins is None:
            xdg_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xdg_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                dogechia.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), dogechia.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, dogechia.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the xdg spend to fund the minting.
        spend_value = sum([coin.amount for coin in xdg_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xdg_spends = []
        if xdg_change_ph is None:
            xdg_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xdg_payment = Payment(xdg_change_ph, change, [xdg_change_ph])

        first = True
        for xdg_coin in xdg_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xdg_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xdg_coins]
                message_list.append(Coin(xdg_coin.name(), xdg_payment.puzzle_hash, xdg_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xdg_coins) > 1:
                    xdg_announcement: Optional[Set[bytes]] = {message}
                else:
                    xdg_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[xdg_payment],
                    fee=fee,
                    coin_announcements=xdg_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xdg_coin.name(), message).name()
            ...
            xdg_spends.append(CoinSpend(xdg_coin, puzzle, solution))
        xdg_spend = await self.standard_wallet.sign_transaction(xdg_spends)
        ...
        did_full_puzzle: Program = dogechia.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xdg_spend, *eve_spends])
        return total_spend

    async def mint_from_xdg(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XDG spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xdg_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XDG to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XDG spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XDG spend
            ...
            # Annnouncements for eve spend. These are asserted by the xdg spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xdg_coin in xdg_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[xdg_payment] + primaries,
                    fee=fee,
                    coin_announcements=xdg_announcement if len(xdg_coins) > 1 else None,
