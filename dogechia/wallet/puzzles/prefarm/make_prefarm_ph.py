...
address1 = "txdg15gx26ndmacfaqlq8m0yajeggzceu7cvmaz4df0hahkukes695rss6lej7h"  # Gene wallet (m/12381/8444/2/42):
address2 = "txdg1c2cguswhvmdyz9hr3q6hak2h6p9dw4rz82g4707k2xy2sarv705qcce4pn"  # Mariano address (m/12381/8444/2/0)
...
def make_puzzle(amount: int) -> int:
    ...
    for cvp in parse_sexp_to_conditions(result):
        ...
        total_dogechia += int_from_bytes(cvp.vars[1])
        print(
            f"{ConditionOpcode(cvp.opcode).name}: {encode_puzzle_hash(bytes32(cvp.vars[0]), prefix)},"
            f" amount: {int_from_bytes(cvp.vars[1])}"
        )
    return total_dogechia


total_dogechia = 0
print("Pool address: ")
total_dogechia += make_puzzle(pool_amounts)
print("\nFarmer address: ")
total_dogechia += make_puzzle(farmer_amounts)

assert total_dogechia == calculate_base_farmer_reward(uint32(0)) + calculate_pool_reward(uint32(0))
