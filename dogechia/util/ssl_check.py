...
from dogechia.util.config import load_config, traverse_dict
from dogechia.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run the following to fix issues: dogechia init --fix-ssl-permissions")
