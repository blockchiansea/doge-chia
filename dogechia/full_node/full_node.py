...
from dogechia.consensus.block_creation import unfinished_block_to_full_block
from dogechia.consensus.block_record import BlockRecord
from dogechia.consensus.blockchain import AddBlockResult, Blockchain, StateChangeSummary
...
from dogechia.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
...
from dogechia.consensus.pot_iterations import calculate_sp_iters
from dogechia.full_node.block_store import BlockStore
from dogechia.full_node.bundle_tools import detect_potential_template_generator
from dogechia.full_node.coin_store import CoinStore
from dogechia.full_node.full_node_api import FullNodeAPI
from dogechia.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from dogechia.full_node.hint_management import get_hints_and_subscription_coin_ids
from dogechia.full_node.hint_store import HintStore
from dogechia.full_node.lock_queue import LockClient, LockQueue
from dogechia.full_node.mempool_manager import MempoolManager
from dogechia.full_node.signage_point import SignagePoint
from dogechia.full_node.subscriptions import PeerSubscriptions
from dogechia.full_node.sync_store import SyncStore
from dogechia.full_node.tx_processing_queue import TransactionQueue
from dogechia.full_node.weight_proof import WeightProofHandler
from dogechia.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from dogechia.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from dogechia.protocols.protocol_message_types import ProtocolMessageTypes
from dogechia.protocols.wallet_protocol import CoinState, CoinStateUpdate
from dogechia.rpc.rpc_server import StateChangedProtocol
from dogechia.server.node_discovery import FullNodePeers
from dogechia.server.outbound_message import Message, NodeType, make_msg
from dogechia.server.peer_store_resolver import PeerStoreResolver
from dogechia.server.server import DogechiaServer
...
from dogechia.types.blockchain_format.pool_target import PoolTarget
...
from dogechia.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from dogechia.types.mempool_inclusion_status import MempoolInclusionStatus
from dogechia.types.spend_bundle import SpendBundle
from dogechia.types.transaction_queue_entry import TransactionQueueEntry
from dogechia.types.unfinished_block import UnfinishedBlock
from dogechia.util import cached_bls
from dogechia.util.bech32m import encode_puzzle_hash
from dogechia.util.check_fork_next_block import check_fork_next_block
from dogechia.util.condition_tools import pkm_pairs
from dogechia.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from dogechia.util.db_synchronous import db_synchronous_on
from dogechia.util.db_version import lookup_db_version, set_db_version_async
from dogechia.util.db_wrapper import DBWrapper2, manage_connection
from dogechia.util.errors import ConsensusError, Err, ValidationError
from dogechia.util.ints import uint8, uint32, uint64, uint128
from dogechia.util.limited_semaphore import LimitedSemaphore
from dogechia.util.path import path_from_root
from dogechia.util.profiler import mem_profile_task, profile_task
from dogechia.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    ...
    root_path: Path
    config: Dict[str, Any]
    _server: Optional[DogechiaServer]
    _shut_down: bool
    ...
    @property
    def server(self) -> DogechiaServer:
                ...
                async with conn.execute(
                    ...
                    if len(list(await cur.fetchall())) == 0:
                        try:
                            ...
                            # it could be a database created with "dogechia init", which is
    ...
    def set_server(self, server: DogechiaServer) -> None:
        self._server = server
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.dogechi.org")
        try:
    ...
    async def short_sync_batch(self, peer: WSDogechiaConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSDogechiaConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSDogechiaConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSDogechiaConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSDogechiaConnection) -> None:
        """
    ...
    def on_disconnect(self, connection: WSDogechiaConnection) -> None:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[WSDogechiaConnection] = [
            ...
            weight_proof_peer: WSDogechiaConnection = random.choice(peers_with_peak)
            self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSDogechiaConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSDogechiaConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSDogechiaConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSDogechiaConnection, List[FullBlock]]]]
        ) -> None:
            advanced_peak: bool = False
            while True:
                res: Optional[Tuple[WSDogechiaConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSDogechiaConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSDogechiaConnection]:
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSDogechiaConnection = self.server.all_connections[peer]
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSDogechiaConnection] = None,
    ...
    async def add_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'xdg')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSDogechiaConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSDogechiaConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSDogechiaConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSDogechiaConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSDogechiaConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSDogechiaConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Optional[Any] = await peer.call_api(
