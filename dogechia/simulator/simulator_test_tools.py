...
from dogechia.daemon.server import WebSocketServer, daemon_launch_lock_path
from dogechia.simulator.full_node_simulator import FullNodeSimulator
...
from dogechia.simulator.start_simulator import async_main as start_simulator_main
...
from dogechia.util.config import create_default_dogechia_config, load_config, save_config
from dogechia.util.errors import KeychainFingerprintExists
from dogechia.util.ints import uint32
from dogechia.util.keychain import Keychain
from dogechia.util.lock import Lockfile
from dogechia.wallet.derive_keys import master_sk_to_wallet_sk
...
def create_config(
    ...
    # create dogechia directories
    create_default_dogechia_config(dogechia_root)
    create_all_ssl(
        dogechia_root,
    ...
    # load config
    config = load_config(dogechia_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txdg")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(dogechia_root, "config.yaml", config)
    return config


async def start_simulator(dogechia_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=dogechia_root)
    await service.start()
...
async def get_full_dogechia_simulator(
    dogechia_root: Path,
    ...
    """
    A dogechia root Path is required.
    The dogechia root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the dogechia test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(dogechia_root)):
        ...
        crt_path = dogechia_root / config["daemon_ssl"]["private_crt"]
        key_path = dogechia_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = dogechia_root / config["private_ssl_ca"]["crt"]
        ca_key_path = dogechia_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(dogechia_root, ca_crt_path, ca_key_path, crt_path, key_path)
        await ws_server.setup_process_global_state()
        async with ws_server.run():
            async for simulator in start_simulator(dogechia_root, automated_testing):
                yield simulator, dogechia_root, config, mnemonic, fingerprint, keychain
