...
from dogechia.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from dogechia.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from dogechia.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from dogechia.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from dogechia.wallet.sign_coin_spends import sign_coin_spends
from dogechia.wallet.trading.offer import NotarizedPayment, Offer
...
from dogechia.wallet.util.merkle_utils import _simplify_merkle_proof
from dogechia.wallet.util.transaction_type import TransactionType
from dogechia.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSDogechiaConnection) -> CoinState:
    ...
    async def create_tandem_xdg_tx(
        self,
        fee: uint64,
        announcement_to_assert: Announcement,
        coin_announcement: bool = True,
    ) -> TransactionRecord:
        dogechia_tx = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            dogechia_tx = await self.create_tandem_xdg_tx(
                fee, Announcement(current_coin.name(), b"$"), coin_announcement=True
            )
            assert dogechia_tx.spend_bundle is not None
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, dogechia_tx.spend_bundle])
            ...
            txs: List[TransactionRecord] = [dl_tx, dogechia_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSDogechiaConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            dogechia_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, dogechia_tx.spend_bundle])
                ),
                dataclasses.replace(dogechia_tx, spend_bundle=None),
            ]
