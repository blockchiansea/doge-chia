...
from dogechia.rpc.util import wrap_http_handler
from dogechia.server.outbound_message import NodeType
from dogechia.server.server import DogechiaServer, ssl_context_for_client, ssl_context_for_server
...
from dogechia.util.config import str2bool
from dogechia.util.ints import uint16
from dogechia.util.json_util import dict_to_json_str
from dogechia.util.network import WebServer, resolve
from dogechia.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> DogechiaServer:
        ...
        # a property so as to be read only which allows DogechiaServer to satisfy
        # Optional[DogechiaServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        dogechia.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: DogechiaServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
