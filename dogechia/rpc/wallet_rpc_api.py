...
from dogechia.consensus.block_rewards import calculate_base_farmer_reward
from dogechia.data_layer.data_layer_errors import LauncherCoinNotFoundError
from dogechia.data_layer.data_layer_wallet import DataLayerWallet
from dogechia.pools.pool_wallet import PoolWallet
from dogechia.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from dogechia.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from dogechia.server.outbound_message import NodeType, make_msg
...
from dogechia.types.blockchain_format.coin import Coin, coin_as_list
...
from dogechia.types.coin_spend import CoinSpend
from dogechia.types.signing_mode import SigningMode
from dogechia.types.spend_bundle import SpendBundle
from dogechia.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from dogechia.util.byte_types import hexstr_to_bytes
from dogechia.util.config import load_config, str2bool
from dogechia.util.errors import KeychainIsLocked
from dogechia.util.ints import uint16, uint32, uint64
from dogechia.util.keychain import bytes_to_mnemonic, generate_mnemonic
from dogechia.util.misc import UInt32Range
from dogechia.util.path import path_from_root
from dogechia.util.streamable import Streamable, streamable
from dogechia.util.ws_message import WsRpcMessage, create_payload_dict
from dogechia.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from dogechia.wallet.cat_wallet.cat_wallet import CATWallet
...
from dogechia.wallet.did_wallet.did_wallet import DIDWallet
from dogechia.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    match_did_puzzle,
    metadata_to_program,
    program_to_metadata,
...
from dogechia.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from dogechia.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from dogechia.wallet.nft_wallet.nft_wallet import NFTWallet
from dogechia.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from dogechia.wallet.notification_store import Notification
from dogechia.wallet.outer_puzzles import AssetType
from dogechia.wallet.payment import Payment
from dogechia.wallet.puzzle_drivers import PuzzleInfo, Solver
from dogechia.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from dogechia.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from dogechia.wallet.singleton import create_singleton_puzzle
from dogechia.wallet.trade_record import TradeRecord
from dogechia.wallet.trading.offer import Offer
...
from dogechia.wallet.util.address_type import AddressType, is_valid_address
from dogechia.wallet.util.compute_hints import compute_coin_hints
from dogechia.wallet.util.compute_memos import compute_memos
from dogechia.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from dogechia.wallet.util.transaction_type import CLAWBACK_TRANSACTION_TYPES, TransactionType
from dogechia.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from dogechia.wallet.util.wallet_types import CoinType, WalletType
from dogechia.wallet.vc_wallet.vc_store import VCProofs
from dogechia.wallet.vc_wallet.vc_wallet import VCWallet
...
from dogechia.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords
from dogechia.wallet.wallet_info import WalletInfo
from dogechia.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "dogechia_wallet"
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSDogechiaConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xdg_target_address")
        pool_target = config["pool"].get("xdg_target_address")
    ...
    async def create_new_wallet(self, request: Dict) -> EndpointResult:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from dogechia.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        # signatures made by `dogechia keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
        ...
        if signing_mode == SigningMode.CHIP_0002:
            ...
            #   ("Dogechia Signed Message", message)
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from dogechia.util.bech32m import bech32_decode, convertbits
        from dogechia.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xdg")
        ...
        key = None
        if asset_id is not None and asset_id != "xdg":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XDG.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xdg_coin_list = request.get("xdg_coins", None)
        xdg_coins = None
        if xdg_coin_list:
            xdg_coins = set([Coin.from_json_dict(xdg_coin) for xdg_coin in xdg_coin_list])
        xdg_change_target = request.get("xdg_change_target", None)
        if xdg_change_target is not None:
            if xdg_change_target[:2] == "xdg":
                xdg_change_ph = decode_puzzle_hash(xdg_change_target)
            else:
                xdg_change_ph = bytes32(hexstr_to_bytes(xdg_change_target))
        else:
            xdg_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_xdg(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
