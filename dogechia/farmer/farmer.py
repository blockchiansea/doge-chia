...
from dogechia.plot_sync.delta import Delta
from dogechia.plot_sync.receiver import Receiver
from dogechia.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
from dogechia.protocols import farmer_protocol, harvester_protocol
from dogechia.protocols.pool_protocol import (
...
from dogechia.server.server import DogechiaServer, ssl_context_for_root
from dogechia.server.ws_connection import WSDogechiaConnection
from dogechia.ssl.create_ssl import get_mozilla_ca_crt
from dogechia.types.blockchain_format.proof_of_space import ProofOfSpace
...
from dogechia.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from dogechia.util.errors import KeychainProxyConnectionFailure
from dogechia.util.hash import std_hash
from dogechia.util.ints import uint8, uint16, uint64
from dogechia.util.keychain import Keychain
from dogechia.util.logging import TimedDuplicateFilter
...
from dogechia.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'dogechia keys generate' or open the UI."
        try:
        ...
        if "xdg_target_address" not in self.config:
            self.config = config["farmer"]
        if "xdg_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xdg_target_address" not in self.config or "xdg_target_address" not in self.pool_config:
            log.debug("xdg_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["xdg_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["xdg_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["xdg_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["xdg_target_address"] = pool_target_encoded
