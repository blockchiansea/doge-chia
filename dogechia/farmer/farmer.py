...
import dogechia.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from dogechia.pools.pool_config import PoolWalletConfig, load_pool_config
...
from dogechia.util.config import load_config, save_config, config_path_for_filename
from dogechia.util.hash import std_hash
from dogechia.util.ints import uint8, uint16, uint32, uint64
...
class Farmer:
    def __init__(
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'dogechia keys generate' or open the UI."
        ...
        self.pool_target_encoded = pool_config["xdg_target_address"]
    ...
    def on_disconnect(self, connection: ws.WSDogeChiaConnection):
    ...
    async def get_cached_harvesters(self, connection: WSDogeChiaConnection) -> HarvesterCacheEntry:
