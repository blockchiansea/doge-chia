...
from dogechia.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from dogechia.plotters.chiapos import get_chiapos_install_info, plot_dogechia
from dogechia.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in DOGECHIA_ROOT.
    dogechia_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within DOGECHIA_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("dogechia plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default dogechia plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'dogechia plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'dogechia plotters bladebit2' was integrated to 'dogechia plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_dogechia(args, dogechia_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, dogechia_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, dogechia_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(dogechia_root_path)
