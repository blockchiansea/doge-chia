...
from dogechia.cmds.init_funcs import dogechia_full_version_str
from dogechia.daemon.server import service_launch_lock_path
from dogechia.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from dogechia.server.dogechia_policy import set_dogechia_policy
from dogechia.server.outbound_message import NodeType
from dogechia.server.server import DogechiaServer
from dogechia.server.ssl_context import dogechia_ssl_ca_paths, private_ssl_ca_paths
from dogechia.server.upnp import UPnP
from dogechia.server.ws_connection import WSDogechiaConnection
from dogechia.types.peer_info import PeerInfo, UnresolvedPeerInfo
from dogechia.util.ints import uint16
from dogechia.util.lock import Lockfile, LockfileError
from dogechia.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSDogechiaConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"doge-chia version: {dogechia_full_version_str()}H")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        dogechia_ca_crt, dogechia_ca_key = dogechia_ssl_ca_paths(root_path, self.config)
        ...
        self._server = DogechiaServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (dogechia_ca_crt, dogechia_ca_key),
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"dogechia_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for DogechiaServer to be closed")
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_dogechia_policy(connection_limit)
