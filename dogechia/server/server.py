...
from dogechia.protocols.protocol_state_machine import message_requires_reply
from dogechia.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from dogechia.server.ws_connection import ConnectionCallback, WSDogechiaConnection
...
from dogechia.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from dogechia.util.ssl_check import verify_ssl_certs_and_keys
...
class DogechiaServer:
    ...
    all_connections: Dict[bytes32, WSDogechiaConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        dogechia_ca_crt_path, dogechia_ca_key_path = dogechia_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=dogechia_ca_crt_path,
                ca_key=dogechia_ca_key_path,
        ...
        return cls(
            ...
            _network_id='dogechia-' + network_id,
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("DogechiaServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSDogechiaConnection.create(
    ...
    async def connection_added(
        self, connection: WSDogechiaConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSDogechiaConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSDogechiaConnection, trusted_peers: Dict[str, Any]) -> bool:
