...
from dogechia.protocols.full_node_protocol import RequestPeers, RespondPeers
from dogechia.protocols.introducer_protocol import RequestPeersIntroducer
...
from dogechia.server.address_manager_sqlite_store import create_address_manager_from_db
...
from dogechia.types.peer_info import PeerInfo, TimestampedPeerInfo, UnresolvedPeerInfo
from dogechia.util.hash import std_hash
from dogechia.util.ints import uint16, uint64
from dogechia.util.network import IPAddress, resolve
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: DogechiaServer = server
    ...
    async def update_peer_timestamp_on_message(self, peer: WSDogechiaConnection) -> None:
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self) -> None:
        ...
        async def on_connect(peer: WSDogechiaConnection) -> None:
    ...
    async def on_connect_callback(self, peer: WSDogechiaConnection) -> None:
