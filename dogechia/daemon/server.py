...
from dogechia.server.server import ssl_context_for_root, ssl_context_for_server
from dogechia.ssl.create_ssl import get_mozilla_ca_crt
from dogechia.util.dogechia_logging import initialize_logging
...
service_plotter = "dogechia plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("dogechia daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with DOGECHIA_ROOT: {os.environ['DOGECHIA_ROOT']}")
