...
from dogechia.cmds.init_funcs import check_keys, dogechia_full_version_str, dogechia_init
from dogechia.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from dogechia.daemon.keychain_server import KeychainServer, keychain_commands
from dogechia.daemon.windows_signal import kill
from dogechia.plotters.plotters import get_available_plotters
from dogechia.plotting.util import add_plot_directory
from dogechia.server.server import ssl_context_for_server
from dogechia.util.beta_metrics import BetaMetricsLogger
from dogechia.util.dogechia_logging import initialize_service_logging
from dogechia.util.config import load_config
from dogechia.util.errors import KeychainCurrentPassphraseIsInvalid
from dogechia.util.json_util import dict_to_json_str
from dogechia.util.keychain import Keychain, passphrase_requirements, supports_os_passphrase_storage
from dogechia.util.lock import Lockfile, LockfileError
from dogechia.util.network import WebServer
from dogechia.util.service_groups import validate_service
from dogechia.util.setproctitle import setproctitle
from dogechia.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Dogechia.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "dogechia": "dogechia",
        "dogechia_data_layer": "start_data_layer",
        "dogechia_data_layer_http": "start_data_layer_http",
        "dogechia_wallet": "start_wallet",
        "dogechia_full_node": "start_full_node",
        "dogechia_harvester": "start_harvester",
        "dogechia_farmer": "start_farmer",
        "dogechia_introducer": "start_introducer",
        "dogechia_timelord": "start_timelord",
        "dogechia_timelord_launcher": "timelord_launcher",
        "dogechia_full_node_simulator": "start_simulator",
        "dogechia_seeder": "start_seeder",
        "dogechia_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Dogechia will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["dogechia", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'dogechia plotters bladebit ramplot' or 'dogechia plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def exit(self) -> None:
        ...
        log.info("dogechia daemon exiting")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered DOGECHIA_ROOT
    os.environ["DOGECHIA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with DOGECHIA_ROOT: {os.environ['DOGECHIA_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in dogechia_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    dogechia_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("dogechia_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"doge-chia version: {dogechia_full_version_str()}H")
