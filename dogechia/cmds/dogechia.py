...
from dogechia.cmds.beta import beta_cmd
from dogechia.cmds.completion import completion
from dogechia.cmds.configure import configure_cmd
from dogechia.cmds.data import data_cmd
from dogechia.cmds.db import db_cmd
from dogechia.cmds.dev import dev_cmd
...
from dogechia.cmds.passphrase import passphrase_cmd
from dogechia.cmds.peer import peer_cmd
from dogechia.cmds.plotnft import plotnft_cmd
from dogechia.cmds.plots import plots_cmd
from dogechia.cmds.plotters import plotters_cmd
from dogechia.cmds.rpc import rpc_cmd
...
from dogechia.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from dogechia.util.errors import KeychainCurrentPassphraseIsInvalid
from dogechia.util.keychain import Keychain, set_keys_root_path
from dogechia.util.ssl_check import check_ssl

CONTEXT_SETTINGS = dict(help_option_names=["-h", "--help"])


@click.group(
    help=f"\n  Manage dogechia blockchain infrastructure ({__version__}H)\n",
...
def cli(
    ...
    if passphrase_file is not None:
        ...
        from dogechia.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", help="Show dogechia version")
def version_cmd() -> None:
    print(__version__+"H")


@cli.command("run_daemon", help="Runs dogechia daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by dogechia start <service>
)
@click.pass_context
