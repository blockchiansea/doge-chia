...
def generate(shell: str) -> None:
    ...
    You can source it directly (performs slower) by running:
        \033[3;33meval "$(dogechia complete generate)"\033[0m
    or you can save the output to a file:
        \033[3;33mdogechia complete generate > ~/.dogechia-complete-bash\033[0m
    and source that file with:
        \033[3;33m. ~/.dogechia-complete-bash\033[0m
    """
    ...
    subprocess.run(["dogechia"], check=True, env={**os.environ, "_DOGECHIA_COMPLETE": f"{shell}_source"})
