...
from dogechia.cmds.passphrase_funcs import get_current_passphrase
from dogechia.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from dogechia.util.errors import KeychainMaxUnlockAttempts
from dogechia.util.keychain import Keychain
from dogechia.util.service_groups import services_for_groups
...
async def async_start(root_path: Path, config: Dict[str, Any], group: tuple[str, ...], restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the dogechia daemon")
        return None
