...
from dogechia.daemon.client import acquire_connection_to_daemon
from dogechia.util.errors import KeychainMaxUnlockAttempts
from dogechia.util.keychain import Keychain, supports_os_passphrase_storage
from dogechia.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE, KeyringWrapper
from dogechia.util.misc import prompt_yes_no
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Dogechia processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("DOGECHIA_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'dogechia passphrase set' or 'dogechia passphrase remove' to update or remove your passphrase")
        sys.exit(1)
