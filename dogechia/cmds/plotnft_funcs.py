...
from dogechia.cmds.cmds_util import get_any_service_client, transaction_status_msg, transaction_submitted_msg
from dogechia.cmds.units import units
from dogechia.cmds.wallet_funcs import print_balance, wallet_coin_unit
from dogechia.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from dogechia.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
from dogechia.protocols.pool_protocol import POOL_PROTOCOL_VERSION
from dogechia.rpc.farmer_rpc_client import FarmerRpcClient
...
async def create(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["dogechia"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  dogechia start -r farmer")
        return
    print("Aborting.")
