...
from dogechia.cmds.cmds_util import get_any_service_client
from dogechia.cmds.start_funcs import async_start
from dogechia.consensus.coinbase import create_puzzlehash_for_pk
from dogechia.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from dogechia.util.config import load_config, save_config
...
def create_dogechia_directory(
    ...
    """
    This function creates a new dogechia directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from dogechia.cmds.init_funcs import dogechia_init

    if not dogechia_root.is_dir() or not Path(dogechia_root / "config" / "config.yaml").exists():
        # create dogechia directories & load config
        dogechia_init(dogechia_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(dogechia_root, "config.yaml")
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `dogechia keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from dogechia.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from dogechia.simulator.block_tools import BlockTools, test_constants
    from dogechia.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["DOGECHIA_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create dogechia directory & get config.
    print("Creating dogechia directory & config...")
    config = create_dogechia_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "dogechia")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your DOGECHIA_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from dogechia.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        if node_client is not None:
            ...
            # chain status ( basically dogechia show -s)
            ...
            print(
                ...
                f"with a balance of: "
                f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['dogechia']} TXDG."
            )
            if show_addresses:
                print("All Addresses: ")
