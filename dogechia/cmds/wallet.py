...
from dogechia.cmds.check_wallet_db import help_text as check_help_text
from dogechia.cmds.cmds_util import execute_with_wallet
from dogechia.cmds.coins import coins_cmd
from dogechia.cmds.plotnft import validate_fee
from dogechia.wallet.transaction_sorting import SortKey
...
@wallet_cmd.command("send", help="Send dogechia to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much XDG or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much XDG or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in XDG", default="0", show_default=True
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XDG}
    import asyncio
...
@wallet_cmd.command("make_offer", help="Create an offer of XDG/CATs/NFTs for XDG/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in XDG", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in XDG", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Dogechia Wallet DBs"""
    import asyncio

    from dogechia.cmds.check_wallet_db import scan
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XDG.",
    type=str,
    default="0",
    show_default=True,
...
@click.option("-m", "--fee", help="The fee for the transaction, in XDG", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in XDG", type=str, required=False)
def _mint_vc(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option("-m", "--fee", help="Blockchain fee for update transaction, in XDG", type=str, required=False)
@click.option(
...
@click.option("-m", "--fee", help="Blockchain fee for revocation transaction, in XDG", type=str, required=False)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
    default=False,
