...
@click.option("-a", "--amount", help="How much dogechia to destroy to create this coloured coin, in TXDG/XDG", type=str)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XDG",
...
def create_coloured_coin_cmd(wallet_rpc_port: int, colour_id: str, amount: str, fee: str, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(Decimal(fee) * units["dogechia"]))
    ...
    if colour_id:
        ...
        data["mode"] = "new"
        final_amount = uint64(int(Decimal(amount) * units["dogechia"]))
...
@click.option("-l", "--limit", help="Spendable amount per interval, in TXDG/XDG.", type=str, required=True)
@click.option("-a", "--amount", help="Amount for initial coin. in TXDG/XDG.", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XDG",
...
def create_rate_limited_admin_cmd(
    ...
    final_fee = uint64(int(Decimal(fee) * units["dogechia"]))
    final_amount = uint64(int(Decimal(amount) * units["dogechia"]))
    final_limit = uint64(int(Decimal(limit) * units["dogechia"]))
    data: Dict[str, Any] = {
        "fee": final_fee,
        "amount": final_amount,
        "pubkey": public_key,
