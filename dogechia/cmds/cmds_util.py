...
from dogechia.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
from dogechia.rpc.data_layer_rpc_client import DataLayerRpcClient
...
from dogechia.rpc.harvester_rpc_client import HarvesterRpcClient
...
from dogechia.types.mempool_submission_status import MempoolSubmissionStatus
...
from dogechia.util.keychain import KeyData
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'dogechia wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                print("No keys loaded. Run 'dogechia keys generate' or import a key")
